#ifndef SHADER_PATHS_H_SEEN
#define SHADER_PATHS_H_SEEN

extern const char* const FRAG_EQUALS_MODULATE_UNIFORM_FRAG;
extern const char* const GL_POSITION_EQUALS_UV_2;

extern const char* const ATLAS_MODULATE_VERT;
extern const char* const ATLAS_MODULATE_FRAG;
extern const char* const ATLAS_MODULATE_MONOCHROME_FRAG;

extern const char* const BASIC2D_MODULATE_VERT;
extern const char* const BASIC2D_MODULATE_FRAG;

extern const char* const LINE_FROM_XY_VERT;
extern const char* const LINE_FROM_XY_VERTEX_VERT;
extern const char* const LINE_FROM_XY_POINT_UNIFORM_OFFSET_VERT;


#endif